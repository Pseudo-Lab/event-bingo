...
test("landing page keeps the experience and admin entry points prominent", async ({ page }) => {
  ...
  await expect(page.getByLabel("예상 행사 날짜")).toBeVisible();
  ...
  await expect(page.getByRole("button", { name: "신청하기" })).toHaveCount(1);
  await expect(
  ...
  await page.getByLabel("예상 행사 날짜").fill("2026-06-10");
  await page.getByLabel("예상 참가자 수").selectOption("200");
  await page.getByRole("button", { name: "신청하기" }).click();
  await expect(page.getByRole("dialog")).toContainText("신청이 접수되었습니다");
  await expect(page.getByRole("status")).toContainText("입력하신 이메일로 접수 확인 메일을 발송했습니다.");
