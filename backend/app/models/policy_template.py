...
DevFactory 서비스 운영팀은 행사 페이지와 관리자 도구를 제공하는 운영 지원 주체이며, 서비스 운영팀의 개인정보 처리 기준은 별도 공개된 Bingo Networking 개인정보처리방침(**{platformPrivacyPath}**)을 따릅니다.
...
■ 2. 처리하는 개인정보 항목과 목적
\\- **참가자 식별 및 로그인**
  \\- 항목: 이름, 이메일 주소, 로그인 ID, Google/Supabase 식별자, 서비스 내부 사용자 ID
  \\- 목적: 참가자 본인 확인, 행사별 참가 이력 관리, 중복 로그인 방지
\\- **빙고 서비스 운영**
...
DEFAULT_PLATFORM_POLICY_TEMPLATE = """# Bingo Networking 개인정보처리방침

DevFactory 서비스 운영팀은 Bingo Networking 서비스와 관리자 도구를 운영합니다.
...
향후 데이터 분석, 추천, 연구/통계, 고도화된 리포트 기능이 도입되는 경우, 개인을 식별할 수 있는 정보의 추가 처리 여부, 처리 목적, 처리 항목, 보유기간, 제3자 제공 또는 처리위탁 여부를 별도로 안내하고 필요한 동의 또는 법적 근거를 확인합니다.
...
■ 8. 정보주체의 권리·의무 및 행사방법
정보주체는 개인정보 열람, 정정·삭제, 처리정지 등을 요청할 수 있습니다.
Bingo Networking 개인정보 보호 관련 문의, 열람청구, 정정·삭제 요구는 **{platformContactEmail}**로 할 수 있으며, 법령에서 정한 사유가 없는 한 지체 없이 처리합니다.
