...
DEFAULT_CONSENT_POLICY_TEMPLATE = """# [필수] 개인정보 수집 및 이용 동의서
...
class PolicyTemplate(Base):
    ...
    async def ensure_consent_template(cls, session: AsyncSession) -> "PolicyTemplate":
        existing = await cls.get_by_key(session, CONSENT_POLICY_TEMPLATE_KEY)
        if existing:
            return existing

        template = cls(
            template_key=CONSENT_POLICY_TEMPLATE_KEY,
            content_markdown=DEFAULT_CONSENT_POLICY_TEMPLATE,
        )
    ...
    async def update_consent_template(
        ...
        *,
        ...
        template = await cls.ensure_consent_template(session)
